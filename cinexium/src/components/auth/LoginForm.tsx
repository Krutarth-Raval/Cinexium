  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/send-otp', {
        ...
        body: JSON.stringify({ identifier, action: 'login' }),
      });
      ...
      router.push(`/verify-otp?email=${encodeURIComponent(data.email)}&action=login`);
    } catch (err: any) {
    ...
    <div className="w-full max-w-md p-8 bg-[#0f1115]/80 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl relative z-20 max-h-full overflow-y-auto scrollbar-hide">
      ...
      <div className="mt-8 text-center">
        <p className="text-gray-400 text-sm">
          ...
          <Link href="/register" className="text-primary-500 hover:text-primary-400 font-semibold transition-colors">
            Sign up
          </Link>
        </p>
      </div>
