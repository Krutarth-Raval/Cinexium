...
export const ShareCollectionModal = ({ 
  ...
  const handleCopyLink = () => {
    const link = `${window.location.origin}${shareUrlPath || `/collection/${collectionId}`}`;
  ...
  const renderHeader = () => (
    <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between shrink-0">
      <h3 className="text-xl font-bold text-white">
        {creatorUsername === 'Cinexium:movie' || creatorUsername === 'Cinexium:series' || creatorUsername === 'Cinexium:tv'
  ...
  const renderContent = () => (
    <div className="flex-1 overflow-y-auto px-6 py-5 custom-scrollbar">
      <div>
        <p className="text-sm font-bold text-gray-400 uppercase tracking-[0.24em] mb-4">Share To</p>
        <div
          ...
          {contacts.map(contact => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[11px] leading-4 text-white text-center line-clamp-2 w-full px-1">{displayUser?.name?.split(' ')[0]}</p>
              </div>
            );
          })}
