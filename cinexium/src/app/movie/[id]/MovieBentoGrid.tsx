...
export const MovieBentoGrid = ({ details, region }: { details: any, region?: string }) => {
  ...
  const formattedRuntime = `${runtimeHours > 0 ? `${runtimeHours}h ` : ''}${runtimeMins}m`;

  // Release date for India (or US as fallback)
  let releaseDateString = new Date(details.release_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' });
  let releaseCountry = '';
    ...
    <>
      <div className="w-full pt-4 md:pt-16 pb-4 text-white">
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#1a1d24] border border-white/10 rounded-3xl p-6 shadow-xl relative overflow-hidden h-full">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-y-5 gap-x-4 relative z-10">
                ...
                <div className="border-l-2 border-white/10 pl-3">
                  <h4 className="text-gray-500 font-bold text-[10px] uppercase tracking-widest mb-1">Budget</h4>
                  <p className="font-semibold text-white">{formatCurrency(details.budget)}</p>
                </div>
                <div className="border-l-2 border-white/10 pl-3">
                  <h4 className="text-gray-500 font-bold text-[10px] uppercase tracking-widest mb-1">Revenue</h4>
                  <p className="font-semibold text-white">{formatCurrency(details.revenue)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {recommendations.length > 0 && (
          <section className="mt-16 pb-12 pl-4 sm:pl-6 md:pl-12">
            <h3 className="text-2xl font-bold mb-6 flex items-center gap-2">
              You might also like
            </h3>
